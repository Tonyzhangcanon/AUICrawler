...
- [x] 20. 执行前判断设备状态，如果已锁屏（屏幕未唤醒 & 屏幕唤醒但是锁定状态），则该设备不执行
- [ ] 21. 执行前初始化网络／蓝牙／存储空间
- [x] 22. 执行过程中，当发现被测应用不再运行时，启动被测应用后，会检查设备状态，如果已锁屏（屏幕未唤醒 & 屏幕唤醒但是锁定状态）,则不再继续执行
- [x] 23. 单独设置异常邮件收件人，与结果邮件收件人做区分
- [x] 24. 如果安装被测应用失败，该设备不会执行遍历；
- [x] 25. 如果未开启安装被测应用&被测应用并未安装，则使用Settings中设置的Apk路径尝试安装，如果安装被测应用失败，该设备不会执行遍历；
