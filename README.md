...
- [x] 20. 执行前判断设备状态，是否未唤醒屏幕，是否锁屏
- [ ] 21. 添加指定Activity模式，先随机遍历，进入指定的Actviity后，遍历Activity，中途跳出Activity&非需要遍历的Activity，则找路径还原&返回。还原不了Activity／控件，从未遍历列表移动到未遍历完列表，在报告中显示，直到未遍历列表清空，结束		 +- [ ] 21. 执行前初始化网络／蓝牙／存储空间
- [ ] 22. 执行过程中设备低电量 ／ 关机 ，异常状态处理。
