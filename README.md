...
##### 设置项详解:
```
JenkinsHome: Jenkins的url，如192.168.2.55:8080 ,用于集成到Jenkins后，发送邮件时拼接超链
...
- [ ] 19. 记录&统计执行过程中设备的 内存&CPU等 性能变化
- [ ] 20. 完全随机模式，去除优先点击新产生的控件，完全随机，只过滤掉已点击的控件
- [ ] 21. 添加指定Activity模式，先随机遍历，进入指定的Actviity后，遍历Activity，中途跳出Activity&非需要遍历的Activity，则找路径还原&返回。还原不了Activity／控件，从未遍历列表移动到未遍历完列表，在报告中显示，直到未遍历列表清空，结束
